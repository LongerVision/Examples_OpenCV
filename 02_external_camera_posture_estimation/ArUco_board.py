...
board = aruco.GridBoard_create(5, 7, 0.40, 0.08, aruco_dict)
...
# List video file
videoFile = "/media/peijia/cinetec/Databases/cinetech/calibration/videos/aruco_board_57.mp4"
# List image names
...
# i = 0
    ...
    if ret == True:
        ...
        cv2.imwrite("arucoLeft.jpg", imLeftRemapped_color)
        cv2.imwrite("arucoRight.jpg", imRightRemapped_color)
        ...
        # Estimate board pose
        if idsLeft != None:
            im_with_aruco_left = aruco.drawDetectedMarkers(imLeftRemapped_color, cornersLeft, idsLeft, (0,255,0))
            retvalLeft, rvecLeft, tvecLeft = aruco.estimatePoseBoard(cornersLeft, idsLeft, board, camera_matrix_l, distortion_coefficients_l)
            if retvalLeft != 0:
                im_with_aruco_left = aruco.drawAxis(im_with_aruco_left, camera_matrix_l, distortion_coefficients_l, rvecLeft, tvecLeft, 1)
        else:
            im_with_aruco_left = imLeftRemapped_color

        if idsRight != None:
            im_with_aruco_right = aruco.drawDetectedMarkers(imRightRemapped_color, cornersRight, idsRight, (0,255,0))
            ...
            if retvalRight != 0:
                im_with_aruco_right = aruco.drawAxis(im_with_aruco_right, camera_matrix_r, distortion_coefficients_r, rvecRight, tvecRight, 1)
        else:
